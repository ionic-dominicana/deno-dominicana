...
class TokenService {
     ...
     * @returns Promise<TokenStructure | Error> Returns access and refresh tokens with expiry
     */
    public static async generateRefreshTokensService(userId?: string): Promise<TokenStructure | Error> {
        if (!userId) {
            return throwError({
                status: Status.NotFound,
                name: "NotFound",
     ...
     */
    public static async verifyTokenService(token: string, type: string): Promise<TokenSchema | Error> {
