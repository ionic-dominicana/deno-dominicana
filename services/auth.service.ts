...
class AuthService {
     ...
     */
    public static async login({ email: currentEmail, password }: { email: string; password: string }
    ): Promise<LoginStructure | Error> {
        const user: UserSchema | undefined = await User.findOne(
            { email: currentEmail, isDisabled: false },
        );

        if (user && user.password && await HashHelper.compare(password, user.password)) {
            const { _id, name, email, role, isDisabled, createdAt, updatedAt }: UserSchema = user;
            const tokens: TokenStructure | Error = await TokenService.generateAuthTokensService(_id.toString());
            return ({
                tokens,
                user: {
                    id: _id.toString(),
     ...
     */
    public static async getRefreshToken(token: string): Promise<TokenStructure | Error> {
        const refreshTokenDoc: TokenSchema | Error = await TokenService.verifyTokenService(token, "refresh");
        if ("user" in refreshTokenDoc) {
            ...
            await TokenService.removeExistingRefreshToken(refreshTokenDoc?._id?.toString());
            return await TokenService.generateRefreshTokensService("id" in user ? user.id : undefined);
        }
        return throwError({
